...
motion_loss_optimizer  = tf.train.RMSPropOptimizer(learning_rate=0.001).minimize(loss =loss_motion,global_step=global_step,var_list = vars_to_minimize)
...
tf.train.Saver(param_lstm_encoder).restore(sess,tf.train.latest_checkpoint('./weights/lstm_encoder/'))
print colored("loaded pose lstm_encoder weights","green")

tf.train.Saver(param_lstm_decoder).restore(sess,tf.train.latest_checkpoint('./weights/lstm_decoder/'))
...
def train():
    ...
    for iteration_no in range(0,H.max_iterations):
		...
		op_train_dict = sess.run({'loss':loss_motion, 'optim':motion_loss_optimizer, 'summary_op':summary_op, 'g_step': global_step, 'final_pose': pose_recon}, feed_dict=feed_dict)
		#final_pose  is view norm 
		if iteration_no % 10 == 0:

			print "train loss", op_train_dict['loss']

			print "global step", op_train_dict['g_step']

			fig = plt.figure(figsize=(16, 8))

			fig_img = utils.gen_plot(fig, train_batch_view_norm[:H.seq_length].reshape([H.seq_length, H.num_joints, 3]), op_train_dict['final_pose'][:H.seq_length].reshape([H.seq_length, H.num_joints, 3]), az=90)
...
def val(iteration_no):
		...
		val_batch = val_batch.reshape((-1,15,3))

		angles,val_batch_view_norm,val_batch_local = transform_util.root_relative_to_local_skeleton_batch(val_batch)

		feed_dict = {x_input : val_batch}

		op_val_dict = sess.run({'loss':loss_motion,'summary_op':summary_op, 'g_step': global_step, 'final_pose': pose_recon}, feed_dict=feed_dict)

		fig = plt.figure(figsize=(16, 8))   

		fig_img = utils.gen_plot(fig, val_batch_view_norm[:H.seq_length].reshape([H.seq_length, H.num_joints, 3]), op_val_dict['final_pose'][:H.seq_length].reshape([H.seq_length, H.num_joints, 3]), az=90)
		...
		print "val loss", op_val_dict['loss']

		print "global step", op_val_dict['g_step']
