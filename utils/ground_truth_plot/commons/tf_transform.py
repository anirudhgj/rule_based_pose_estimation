...
a = tf.constant(sk_data.arbitary_vec, dtype=tf.float64)
D = tf.constant(sk_data.D, dtype=tf.float64)
eps = tf.constant(1e-9, dtype=tf.float64)
print a
print D
...
def prior_estimate_absolute_positions(dS):
    ...
    S[limb_order[0]] = tf.zeros((batch_size, 3), dtype=tf.float64)
