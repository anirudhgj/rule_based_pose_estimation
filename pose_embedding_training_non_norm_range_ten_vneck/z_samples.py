...
# M.load_weights(235001, session_1)
...
def pose_rotate(points):
    theta = np.random.randint(-180.0, 180.0, (128,1))
    ...
    zero_size_row1x2 = np.zeros((64,1,2))
    r1x2xZero = np.concatenate((row_12,zero_size_row1x2), axis=1)
    zero_cols = np.zeros((64,3,1))
    stacker = np.array([0.0, 0.0, 1.0])
    third_cols = np.reshape(np.tile(stacker, [64]), [64, 3])
...
print colored('Loaded weights',"yellow")


# print colored(data_loader.num_train_batches(),"blue")
...
for batch_idx in range(num_batches_train * 2):
...
#         time_after_load = time.time()
    ...
    #     if flag_only_x == True and iteration_no < 4:

    outputs = session_1.run(M.z_real,feed_dict)

    # np.save(str(batch_idx))
    mat_z_prior.append(outputs)  

mat_z_prior = np.concatenate(mat_z_prior,axis = 0)
np.save("./save_zs/z_samples.npy",mat_z_prior)
print colored("numpy saved","magenta")
