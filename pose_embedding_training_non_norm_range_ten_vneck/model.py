...
# input_z_real = tf.placeholder(tf.float32, shape=[256, num_zdim], name='input_real_z_joints')
# input_angles_z = tf.placeholder(tf.float32, shape=[256, 3], name='input_decoder_z_angles')
...
# weight_joint_loss = tf.placeholder(tf.float32, shape=[None,17,3 ], name='weight_joint_loss')
...
#Rahul's changes#
# x_recon = comps.local_to_view_norm(x_local_recon)
...
####################################################################################
############################### Disc for z #########################################
# disc_real_z_logits = ZDiscriminatorNet(input_z_real)

# ### Disc for z_hat ###
# disc_fake_z_logits = ZDiscriminatorNet(z_real)


# loss_disc_z_real = tf.reduce_mean(tf.abs(disc_real_z_logits - tf.ones_like(disc_real_z_logits)))
# loss_disc_z_fake = tf.reduce_mean(tf.abs(disc_fake_z_logits + tf.ones_like(disc_fake_z_logits)))

# loss_gen_z = tf.reduce_mean(tf.abs(disc_fake_z_logits - tf.ones_like(disc_fake_z_logits)))

# loss_disc_z_adv = (loss_disc_z_real + loss_disc_z_fake) / 2.

# disc_z_acc_real = tf.reduce_mean(tf.cast(disc_real_z_logits >= 0, tf.float32)) * 100.0
# disc_z_acc_fake = tf.reduce_mean(tf.cast(disc_fake_z_logits < 0, tf.float32)) * 100.0

# disc_z_acc = (disc_z_acc_real + disc_z_acc_fake) / 2.
...
# x_recon_loss_l1_only_left = tf.reduce_mean(tf.abs(x_recon_resnet[:,4, :] - x_real[:,4, :])) + tf.reduce_mean(tf.abs(x_recon_resnet[:,11, :] - x_real[:,11, :])) 
# x_recon_loss_l1_only_right =  tf.reduce_mean(tf.abs(x_recon_resnet[:,7, :] - x_real[:,7, :])) + tf.reduce_mean(tf.abs(x_recon_resnet[:,14, :] - x_real[:,14, :]))
...
scalars = [
    ...
    # tf.summary.scalar('loss_disc_z_real', loss_disc_z_real),
    # tf.summary.scalar('loss_disc_z_fake', loss_disc_z_fake),
    ...
    # tf.summary.scalar('disc_z_acc',disc_z_acc),
    # tf.summary.scalar('disc_z_fake',disc_z_acc_fake),
    # tf.summary.scalar('disc_z_real',disc_z_acc_real),
    ...
    # tf.summary.scalar('loss_gen_z', loss_gen_z),
...
# z_k_histograms = [
#     tf.summary.histogram('z_k_%02d' % i, input_z_real[:, i]) for i in range(num_zdim)
# ]

###################   content loss summaries   ####################################################
# summary_merge_all = tf.summary.merge(scalars  + [tf.summary.histogram('dets', determinants)])
summary_merge_all = tf.summary.merge(scalars + z_j_histograms )#+ z_k_histograms)
...
def load_weights(iter_no, session, dir='../../pretrained_weights/AE_humans_mads_yt_mpi_l1'):
    ...
    tf.train.Saver(param_disc).restore(session, '%s/disc_iter-%d' % (dir, iter_no))
